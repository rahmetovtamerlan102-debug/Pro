...
# batch добавление эмбеддингов
async def embed_batch_worker():
    """Собирает тексты в пачку и вызывает encode раз в 0.2 секунды"""
    batch = []
    while True:
        try:
...
async def add_embedding_batch(user_id, text, role):
    """Ставит задачу в очередь на добавление эмбеддинга"""
...
async def extract_facts(user_id, user_text, assistant_response):
    """Простой сбор фактов на основе ключевых слов (без LLM, чтобы не тратить токены)"""
    facts = {}
    ...
    for key, val in facts.items():
        await db_add_fact(user_id, key, val)
...
async def summarize_history(history):
    """Сжимает старую историю в одно сообщение system"""
    if len(history) <= 8:
        return history
    old = history[:-6]
    recent = history[-6:]
...
async def ask_ai_raw(model, messages, is_code_request):
    ...
    # Добавляем глобальный system prompt
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        f"🤖 *AI Router v2*\n\n"
        f"🧠 *Персональная память* – запоминаю ваш стиль\n"
        f"⚡ *Ускоренные эмбеддинги* – batch processing\n"
        f"📚 *Сжатие диалогов* – длинные беседы не перегружают память\n"
...
async def handle_message(message: types.Message):
    ...
    async with user_semaphores[user_id]:
        # Сохраняем эмбеддинг через batch
...
async def main():
    global session, embed_batch_task
    session = aiohttp.ClientSession()
    # Запускаем воркер для batch-эмбеддингов
    embed_batch_task = asyncio.create_task(embed_batch_worker())
    # Health check сервер (фон)
    asyncio.create_task(start_web_server())
    await asyncio.sleep(0.5)
    # Воркеры очереди
    for i in range(queue_workers):
        asyncio.create_task(worker(i))
    print("🚀 AI Router v2 запущен | User facts | Batch embeddings | Summarization")
