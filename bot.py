...
async def ask_groq_raw(model, messages, is_code_request=False):
    ...
    if is_code_request:
        system_prompt = {
            "role": "system",
            "content": "Ты профессиональный разработчик. Сначала кратко объясни, что будет делать код, а потом напиши готовый рабочий код. Отвечай на русском. Код пиши на английском (синтаксис). Не используй эмодзи."
        }
    else:
        system_prompt = {
            "role": "system",
            "content": "Ты русскоязычный ассистент. Отвечай ТОЛЬКО на русском языке. Будь кратким, полезным и точным. Не используй эмодзи."
        }
...
async def handle_callback(callback: types.CallbackQuery):
    ...
    if data == "back_to_main":
        ...
        await callback.message.edit_text(f"Главное меню\nТекущая модель: {current_name}", reply_markup=main_keyboard())
        await callback.answer()
        return
...
async def main():
    ...
    print("LLM Hub started | 5 models (Meta, OpenAI, Alibaba, Google, Mistral)")
