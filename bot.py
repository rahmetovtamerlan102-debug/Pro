...
async def db_get_user_model(user_id):
    async with db_lock:
        with sqlite3.connect("bot.db", timeout=30) as conn:
            ...
            row = c.fetchone()
            return row[0] if row else "llama31_8b"
    ...
    async with db_lock:
        with sqlite3.connect("bot.db", timeout=30) as conn:
            ...
            c.execute("INSERT OR IGNORE INTO users (user_id, model, created_at) VALUES (?, ?, ?)",
                      (user_id, "llama31_8b", datetime.now()))
            conn.commit()
...
# ==================== ТОП-20 БЕСПЛАТНЫХ МОДЕЛЕЙ (ПРОВЕРЕННЫЕ) ====================
MODELS = {
    ...
    "nemotron3_super": "nvidia/nemotron-3-super:free",
    "nemotron_nano": "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free",
    "nemotron_ultra": "nvidia/llama-3.1-nemotron-ultra-253b:free",
...
MODEL_NAMES = {
    ...
    "owl_alpha": "Owl Alpha",
    "nemotron3_super": "Nemotron 3 Super",
    "nemotron_nano": "Nemotron 3 Nano",
    "nemotron_ultra": "Nemotron Ultra 253B",
