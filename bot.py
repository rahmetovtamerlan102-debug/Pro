...
async def ask_groq_raw(model, messages, is_code_request=False):
    ...
    if is_code_request:
        system_prompt = {
            "role": "system",
            "content": "Ты профессиональный разработчик. Пиши ТОЛЬКО готовый рабочий код. Никаких объяснений, никаких вопросов, никакого текста до или после кода. Только код. Отвечай строго на русском, но сам код пиши на английском (ключевые слова, синтаксис)."
        }
    else:
        system_prompt = {
            "role": "system",
            "content": "Ты русскоязычный ассистент. Отвечай ТОЛЬКО на русском языке. Никогда не используй английский. Будь кратким, полезным и точным. Не используй эмодзи и смайлики."
        }
...
def format_code_response(text, lang):
    # Просто возвращаем код без обёртки в ```
    return text
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        f"LLM Hub\n\n"
        f"Быстрый доступ к AI моделям\n"
        f"Выберите модель ниже",
...
async def main():
    ...
    print("LLM Hub started | 5 models")
