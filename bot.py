...
# ==================== 5 МОДЕЛЕЙ (ПОЛНЫЕ НАЗВАНИЯ БЕЗ РАЗРАБОТЧИКА) ====================
MODEL_LIST = [
...
async def ask_groq_raw(model, messages, is_code_request=False):
    ...
    if is_code_request:
        system_prompt = {
            "role": "system",
            "content": "Ты профессиональный разработчик. Пиши ТОЛЬКО готовый рабочий код. Никаких объяснений, никаких Fragen. Только код. Отвечай на русском, но код оставляй на английском."
        }
    else:
        system_prompt = {
            "role": "system",
...
async def main():
    ...
    print("🚀 Groq Bot started | 5 models: Llama 70B, DeepSeek, Mixtral, Qwen, Gemma")
