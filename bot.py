...
async def fetch_groq_models():
    ...
    now = time.time()
    if _models_cache and (now - _cache_timestamp) < CACHE_TTL:
        ...
        try:
            async with sess.get(url, headers=headers, timeout=15) as resp:
                ...
                for m in data.get("data", []):
                    model_id = m.get("id", "")
                    if any(x in model_id.lower() for x in ["llama", "mixtral", "gemma", "deepseek", "qwen", "phi"]):
...
async def db_get_user_model(user_id):
    async with db_lock:
        with sqlite3.connect("bot.db", timeout=30) as conn:
            ...
            row = c.fetchone()
            return row[0] if row else "llama-3.3-70b-versatile"
    ...
    async with db_lock:
        with sqlite3.connect("bot.db", timeout=30) as conn:
            ...
            c.execute("INSERT OR IGNORE INTO users (user_id, model, created_at) VALUES (?, ?, ?)",
                      (user_id, "llama-3.3-70b-versatile", datetime.now()))
            conn.commit()
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        f"Список обновляется автоматически\n\n"
...
async def main():
    ...
    print("🚀 Groq Bot started | Dynamic models from API")
