...
async def ask_groq_raw(model, messages, is_code_request=False):
    ...
    if is_code_request:
        system_prompt = {
            "role": "system",
            "content": "Ты профессиональный разработчик. Пиши ТОЛЬКО готовый рабочий код. Никаких объяснений. Только код."
        }
    else:
        system_prompt = {
            "role": "system",
            "content": "Ты русскоязычный ассистент. Отвечай ТОЛЬКО на русском языке. Будь кратким и полезным."
        }
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        f"🤖 Groq AI Router | {len(models)} моделей\n\n"
...
async def main():
    ...
    print("🚀 Groq Bot started | 5 models")
