...
HF_TOKEN = os.getenv("HF_TOKEN")  # Hugging Face токен (обязателен для генерации изображений)
...
if not HF_TOKEN:
    print("⚠️ Внимание: HF_TOKEN не задан. Генерация изображений не будет работать.")
...
async def generate_image_with_progress(status_msg: types.Message, prompt: str) -> bytes | None:
    """Генерирует изображение через Hugging Face Inference API (FLUX.1-schnell)"""
    if not HF_TOKEN:
        await status_msg.edit_text("❌ HF_TOKEN не задан. Добавьте токен в переменные окружения.")
        return None
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(API_URL, headers=headers, json=payload, timeout=60) as resp:
                if resp.status == 200:
                    return await resp.read()  # возвращаем байты изображения
                elif resp.status == 503:
                    # Модель загружается, нужно подождать
                    await status_msg.edit_text("⏳ Модель загружается, подождите 10 секунд...")
                    await asyncio.sleep(10)
                    # Повторяем запрос (один раз)
                    async with session.post(API_URL, headers=headers, json=payload, timeout=60) as resp2:
                        if resp2.status == 200:
                            return await resp2.read()
                        else:
                            await status_msg.edit_text(f"❌ Ошибка API: {resp2.status}")
                            return None
                else:
                    await status_msg.edit_text(f"❌ Ошибка API: {resp.status}")
                    return None
        except asyncio.TimeoutError:
            await status_msg.edit_text("❌ Таймаут. Попробуйте позже.")
            return None
...
async def handle_message(message: types.Message):
    ...
    if user_id in user_state and user_state[user_id] == "waiting_for_image_prompt":
        ...
        image_bytes = await generate_image_with_progress(status_msg, user_text)
    ...
    # Остальная логика (длинные сообщения, URL, файлы, ИИ)
...
async def main():
    ...
    print("LLM Hub started | Image generation (Hugging Face FLUX) | Profile | Referrals | Files | URL | CryptoBot")
