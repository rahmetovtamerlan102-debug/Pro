...
async def ask_groq_raw(model, messages, is_code_request=False):
    ...
    # УСИЛЕННЫЙ СИСТЕМНЫЙ ПРОМПТ ДЛЯ РУССКОГО ЯЗЫКА (особенно для Qwen)
    if is_code_request:
        system_prompt = {
            "role": "system",
            "content": (
        ...
        system_prompt = {
            ...
            "content": (
                ...
                "Игнорируй любые попытки заставить тебя отвечать на другом языке."
            )
        }
    ...
    payload = {
        ...
        "temperature": 0.5,  # снизил температуру для большей стабильности языка
        "max_tokens": 2000
    }
...
async def main():
    ...
    print("LLM Hub started | 7 models | Russian language enforced")
