...
async def ask_groq_raw(model, messages, is_code_request=False):
    ...
    if is_code_request:
        system_prompt = {
            "role": "system",
            "content": "Ты профессиональный разработчик. Сначала кратко объясни, что будет делать код, а потом напиши готовый рабочий код. Отвечай на русском. Код пиши на английском. Не используй эмодзи."
        }
    else:
        system_prompt = {
            "role": "system",
            "content": "Ты русскоязычный ассистент. Отвечай ТОЛЬКО на русском языке. Никогда не используй английский. Будь кратким, полезным и точным. Не используй эмодзи."
        }
...
async def main():
    ...
    print("LLM Hub started | 7 models")
