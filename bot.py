...
# ЧИСТЫЕ НАЗВАНИЯ МОДЕЛЕЙ (без ID, без служебных символов)
MODEL_DISPLAY_NAMES = {
...
async def ask_groq_raw(model, messages, is_code_request=False):
    ...
    # Базовый system prompt (всегда на русском)
    if is_code_request:
        system_prompt = {
            "role": "system",
            "content": "Ты профессиональный разработчик. Пиши ТОЛЬКО готовый рабочий код. Никаких объяснений, никаких вопросов. Только код. Отвечай на русском, но код оставляй на английском (ключевые слова)."
        }
    else:
        system_prompt = {
            "role": "system",
...
async def handle_message(message: types.Message):
    ...
    async with user_semaphores[user_id]:
        # Определяем, запрос на код или нет
        ...
        # Получаем историю (в правильном порядке)
...
async def main():
    ...
    print("🚀 Groq Bot started | Clean model names | Russian fixed")
