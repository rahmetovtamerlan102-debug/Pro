...
# 5 СТАБИЛЬНЫХ БЕСПЛАТНЫХ МОДЕЛЕЙ (FALLBACK)
FALLBACK_MODELS = [
    "moonshotai/kimi-k2.6:free",      # Kimi K2.6 (сильная универсальная)
    "deepseek/deepseek-chat:free",    # DeepSeek Chat (сбалансированная)
    "qwen/qwen3-coder:free",          # Qwen3 Coder (код)
    "meta-llama/llama-3.1-8b-instruct:free",  # Llama 3.1 8B (быстрая)
    "openrouter/owl-alpha:free"       # Owl Alpha (рассуждения, агенты)
]
...
def get_fallback_models():
    """Статический список на случай полной недоступности API"""
    return FALLBACK_MODELS.copy()
...
async def smart_route_model(user_text):
    ...
    if re.search(r'(почему|объясни|как работает|докажи|рассуди)', text_lower):
        return "moonshotai/kimi-k2.6:free"   # Kimi K2.6 отлично подходит для рассуждений
...
async def ask_with_fallback(messages, user_text):
    """Пробует несколько моделей по цепочке: умный выбор → UCB1 → fallback список"""
    # 1. Умный роутинг
    ...
    # 3. Жёсткий fallback из 5 стабильных моделей
...
async def get_models_keyboard():
    ...
    if not models:
        models = get_fallback_models()
    # Показываем не более 12 моделей
    models = models[:12]
    buttons = []
    row = []
    for m in models:
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        "🤖 AI Router v2 | 5 стабильных бесплатных моделей\n\n"
        "• Динамическое обновление моделей из OpenRouter (кэш 30 мин)\n"
        "• Если API недоступен, используется fallback из 5 надёжных моделей:\n"
        "  Kimi K2.6, DeepSeek Chat, Qwen3 Coder, Llama 3.1 8B, Owl Alpha\n"
        "• Умный роутинг: код → Qwen Coder, рассуждения → Kimi K2.6\n"
