...
# ==================== ИНИЦИАЛИЗАЦИЯ ЭМБЕДДИНГОВ (ТЯЖЁЛАЯ, ПОСЛЕ ПОРТА) ====================
async def init_embeddings():
    global embedder, index, metadata, embed_batch_queue
...
async def main():
    ...
    # 1. Запускаем health check сервер (простейший)
    await start_web_server()

    # 2. Даём Render время на детект порта (10 секунд)
    await asyncio.sleep(10)

    # 3. Инициализируем тяжёлые компоненты (эмбеддинги, HNSW)
    await init_embeddings()

    # 4. Инициализируем очередь и сессию
    session = aiohttp.ClientSession()
