...
PORT = int(os.environ.get("PORT", 10000))          # Render ожидает порт 10000 по умолчанию
...
def load_hnsw():
    ...
    if os.path.exists(hnsw_index_file) and len(metadata) > 0:
        ...
        print(f"HNSWLIB загружен: {index.get_current_count()} векторов, {len(metadata)} записей")
    else:
        index = hnswlib.Index(space='cosine', dim=dim)
...
def find_similar_context(user_id, query, top_k=10):
    ...
    for label, dist in zip(labels[0], distances[0]):
        ...
        if idx >= 0 and idx < len(metadata) and metadata[idx]["user_id"] == user_id:
            similarity = 1 - dist
            if similarity > 0.5:
                results.append((similarity, metadata[idx]["text"]))
    return results
...
"""
    try:
        judge_reply = await ask_ai("microsoft/phi-3-mini-128k-instruct:free", [{"role": "user", "content": prompt}], False)
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        f"🤖 *AI Router v7 (HNSW) | Агентная платформа*\n\n"
...
async def start_web_server():
    ...
    await site.start()
    print(f"✅ Health check сервер запущен на порту {PORT} (0.0.0.0)")
...
async def main():
    global session
    session = aiohttp.ClientSession()
    # Запускаем health check сервер (сразу, дожидаемся)
    await start_web_server()
    # Запускаем воркеры очереди
    for i in range(queue_workers):
        asyncio.create_task(worker(i))
    print("🤖 AI Router v7 (HNSW) запущен | Health check на /health")
