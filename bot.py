...
# Состояния пользователей (для режимов)
user_state = {}
...
async def generate_video(prompt: str) -> str | None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(create_url, json=payload, headers=headers, timeout=60) as resp:
            ...
            if not task_id:
                return None
        # 2. Ждём готовности (опрос статуса)
...
async def send_video_to_user(message: types.Message, video_url: str):
    """Отправляет видео (сначала попытка по URL, потом скачивание)"""
    try:
...
async def handle_message(message: types.Message):
    ...
    if user_id in user_state and user_state[user_id] == "waiting_for_image_prompt":
        ...
        image_base64 = await generate_image(user_text)
    ...
    if user_id in user_state and user_state[user_id] == "waiting_for_video_prompt":
        ...
        video_url = await generate_video(user_text)
        await status_msg.delete()
        if video_url:
...
async def main():
    ...
    print("LLM Hub started | Image + Video generation | Profile | Referrals | Files | URL | CryptoBot")
