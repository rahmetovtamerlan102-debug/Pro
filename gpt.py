...
MODELS = {
    "auto": "openrouter/free",                          # автоматический выбор лучшей бесплатной
...
MODEL_NAMES = {
    "auto": "🎲 Автовыбор (лучшая бесплатная)",
...
async def ask_ai(model_id, messages, is_code_request=False):
    ...
    # Если выбран автовыбор — используем openrouter/free
    if model_id == "auto":
        model = "openrouter/free"
    else:
        model = MODELS.get(model_id, "openrouter/free")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers) as resp:
            if resp.status != 200:
                ...
                # Если ошибка 402 (нет денег) — подскажем про бесплатные модели
                if resp.status == 402:
                    return "❌ Ошибка: вы выбрали платную модель. Используйте бесплатные модели из списка (все с пометкой :free). Попробуйте сменить модель через кнопку 🧠"
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        f"🤖 *AI Multi-Model Bot (Бесплатные модели)*\n\n"
        f"Доступно *{len(MODELS)}* бесплатных нейросетей\n"
        f"📌 *Текущая модель:* `{MODEL_NAMES[DEFAULT_MODEL]}`\n\n"
        f"💰 *Стоимость:* 0 рублей\n"
        f"🆓 Все модели с пометкой `:free`\n\n"
        f"🔥 *Особенности:*\n"
...
# ==================== ЗАПУСК ====================
async def main():
    print("🤖 Бот запущен (только бесплатные модели)")
