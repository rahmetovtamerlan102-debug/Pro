...
def add_embedding(user_id, text, role):
    ...
    # HNSW сам нормализует для косинусного расстояния
...
def find_similar_context(user_id, query, top_k=10):
    ...
    for label, dist in zip(labels[0], distances[0]):
        ...
        if idx >= 0 and idx < len(metadata) and metadata[idx]["user_id"] == user_id:
            # Косинусное расстояние (0 = одинаковые) – преобразуем в сходство
            similarity = 1 - dist
            if similarity > 0.5:
...
async def main():
    ...
    print("🤖 AI Router v7 (HNSW) запущен: UCB1, факты, reranker, калькулятор")
